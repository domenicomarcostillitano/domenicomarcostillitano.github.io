...
/* Header (sticky, nav centrata) */
.site-header{
  position:fixed; inset:0 0 auto 0; height:64px;
  display:flex; align-items:center; justify-content:center;
  z-index:60; padding-inline: clamp(1rem, 2vw, 2rem);
...
.site-header nav a{
  ...
  padding:.45rem .9rem; border-radius:999px; border:1px solid transparent; font-weight:600;
}
...
/* HERO */
.hero{
  min-height:90vh; display:grid; align-items:center; padding-top:64px;
  background:
...
.kicker{
  text-transform:uppercase; letter-spacing:.12em; font-size:.8rem; font-weight:700; color:var(--accent-2);
}
.hero h1{
  font-family:Merriweather,serif;
  font-size:clamp(2.2rem,4vw,3.2rem);
  margin:.5rem 0 .9rem; color:var(--ink-2);
  white-space:nowrap;
}
...
.btn{
  ...
  text-decoration:none; padding:.85rem 1.15rem;
...
.btn-primary{
  ...
  color:var(--blue-ink); border-color:#cfe3ff;
}
.btn-secondary{
  background:linear-gradient(180deg, var(--soft-green-1), var(--soft-green-2));
  color:var(--green-ink); border-color:#c9f2e1;
}
.btn:hover{ transform:translateY(-2px); box-shadow:0 10px 26px rgba(15,23,42,.10); }
.btn:active{ transform:none; box-shadow:0 4px 14px rgba(15,23,42,.08); }
.btn:focus-visible{ outline:none; box-shadow:0 0 0 4px rgba(14,165,233,.18), 0 10px 26px rgba(15,23,42,.10); }

/* Portrait circolare */
.hero-visual{
  margin-inline:auto; max-width:420px; aspect-ratio:1/1; border-radius:50%; overflow:hidden;
...
/* Cards */
.cards{ display:grid; grid-template-columns:repeat(12,1fr); gap:1rem; margin-top:1.5rem; }
.card{
...
.box{ background:#fff; border:1px solid var(--line); border-radius:var(--radius); box-shadow:var(--shadow); padding:1.25rem; }
...
<!-- 🔻 RIMOSSO: pill-menu-wrapper (i pulsanti in basso)  -->
